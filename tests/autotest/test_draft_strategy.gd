...
func test_damage_strategy_prefers_a_damage_candidate() -> void:
  ...
  assert_eq(driver.choose_draft([_def(ItemCatalog.Id.ARMOR), _def(ItemCatalog.Id.WEAPON)], []), 1)


func test_block_strategy_prefers_a_block_candidate() -> void:
  var driver := AutoTestDriver.new('block', 1)
  assert_eq(driver.choose_draft([_def(ItemCatalog.Id.WEAPON), _def(ItemCatalog.Id.ARMOR)], []), 1)
...
func test_greedy_synergy_prefers_a_connecting_candidate() -> void:
  ...
  var board: Array = [Item.new(_def(ItemCatalog.Id.POISON_DAGGER), owner)]
  var driver := AutoTestDriver.new('greedy-synergy', 1)
  # WEAPON (no synergy) at 0, AVENGER (synergy) at 1 → pick index 1.
  assert_eq(driver.choose_draft([_def(ItemCatalog.Id.WEAPON), _def(ItemCatalog.Id.AVENGER)], board), 1)


func test_random_is_reproducible_for_a_seed() -> void:
  var offer: Array = [_def(ItemCatalog.Id.WEAPON), _def(ItemCatalog.Id.ARMOR), _def(ItemCatalog.Id.POISON_DAGGER)]
...
func test_strategies_can_diverge() -> void:
  var offer: Array = [_def(ItemCatalog.Id.WEAPON), _def(ItemCatalog.Id.ARMOR)]
