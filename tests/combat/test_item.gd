...
func test_weak_owner_fires_reduced_damage() -> void:
  ...
  var owner := Actor.new()
  var it := Item.new(ItemCatalog.get_def(ItemCatalog.WEAPON), owner)
...
func test_duplicates_tick_independently() -> void:
  var owner := Actor.new()
  var a := Item.new(ItemCatalog.get_def(ItemCatalog.WEAPON), owner)
  var b := Item.new(ItemCatalog.get_def(ItemCatalog.WEAPON), owner)
  a.cooldown.step()
