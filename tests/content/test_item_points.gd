...
func test_single_target_damage_is_one_point_each() -> void:
  ...
  assert_almost_eq(ItemPoints.spend(ItemCatalog.get_def(ItemCatalog.CAPPED_CUDGEL)), 10.0, TOLERANCE)
...
func test_shield_costs_more_than_damage() -> void:
  ...
  assert_almost_eq(ItemPoints.spend(ItemCatalog.get_def(ItemCatalog.FLESH_FEMUR)), expected, TOLERANCE)
...
func test_self_damage_is_a_credit() -> void:
  ...
  assert_lt(ItemPoints.spend(ItemCatalog.get_def(ItemCatalog.FLESH_FLENSING_HOOK)), 0.0)
...
func test_bleed_uses_the_triangular_total() -> void:
  ...
  assert_almost_eq(ItemPoints.spend(ItemCatalog.get_def(ItemCatalog.FLESH_BONE_SPEAR)), expected, TOLERANCE)
...
func test_parked_and_unpriced_effects_add_nothing() -> void:
  ...
  assert_almost_eq(ItemPoints.spend(ItemCatalog.get_def(ItemCatalog.DRUID_STAFF)), 10.0, TOLERANCE)
...
func test_enemy_points_are_health_plus_its_items() -> void:
  var def: EnemyDef = EnemyCatalog.get_def(EnemyCatalog.GRUNT)
  var items: float = 0.0
