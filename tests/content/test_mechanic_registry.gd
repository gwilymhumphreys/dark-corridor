...
## lazily like the StatusRegistry. This run registers attack, heal, shield, poison, burn, bleed
## and regen; crit joins as its mechanic is converted.
...
func test_has_is_true_for_the_registered_ids_and_false_for_the_rest() -> void:
  ...
  assert_false(MechanicRegistry.has('crit'), 'crit is not registered yet')
...
func test_shield_multiplier_defaults_to_one() -> void:
  ...
  assert_almost_eq(MechanicRegistry.shield_multiplier('crit'), 1.0, 0.0001, 'an unknown id uses the default')
