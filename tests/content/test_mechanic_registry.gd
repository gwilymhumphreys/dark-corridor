...
## lazily like the StatusRegistry. This run registers attack, heal, shield, poison, burn and
## regen; bleed and crit join as their mechanics are converted.
...
func test_has_is_true_for_the_registered_ids_and_false_for_the_rest() -> void:
  ...
  assert_false(MechanicRegistry.has('bleed'), 'bleed is not registered yet')
...
func test_shield_multiplier_defaults_to_one() -> void:
  ...
  assert_almost_eq(MechanicRegistry.shield_multiplier('bleed'), 1.0, 0.0001, 'an unknown id uses the default')
