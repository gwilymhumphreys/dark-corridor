...
func test_catalog_builds_the_heal_potion() -> void:
  var d := ConsumableCatalog.get_def(ConsumableCatalog.Id.HEALING_DRAUGHT)
...
func test_throwing_a_heal_potion_heals_the_thrower() -> void:
  ...
  var e := _spawn(100.0, [ItemCatalog.Id.ENEMY_CLAW])
  var cm := _manager(p, [e])
  cm.start()
  var potion := Consumable.new(ConsumableCatalog.get_def(ConsumableCatalog.Id.HEALING_DRAUGHT))
