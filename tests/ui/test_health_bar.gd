...
func test_no_shield_hides_the_readout() -> void:
  ...
  a.take_damage(40.0)
  var bar := _bar_for(a)
  var row: Control = bar.get_node('Bar/Readout/Shield')
  ...
  assert_eq(bar.get_node('Bar/Readout/Label').text, '60', 'the label shows current health only')
...
func test_shield_below_max_health_is_measured_against_max_health() -> void:
  ...
  var bar := _bar_for(a)
  assert_true(bar.get_node('Bar/Readout/Shield').visible, 'the readout shows')
  assert_eq(bar.get_node('Bar/Readout/Shield/Value').text, '25', 'with the shield value')
