...
func rebuild() -> void:
  ...
  # The slot's colour: a mechanic's `Colours` variable (the slot id upper-cased). `charge_time`,
  # `card` and `hp` are not mechanics and have no colour, so the row is left out for them, as the Chip sample.
  ...
  # `charge_time`, `card` and `hp` have no keyword chip, so the samples skip it for them.
...
## The slot as a keyword chip, shown only for slots that are mechanics — `charge_time`, `card` and `hp`
