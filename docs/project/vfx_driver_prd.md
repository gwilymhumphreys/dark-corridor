...
- **Not the UI / input** (a separate inbound layer — [UI PRD](ui_layout_prd.md)) or the corridor renderer (the existing `docs/corridors/` scaling-tile renderer; the `VFX driver` is the *combat* wall over it).
...
Because fire-rate and travel are decoupled (combat_prd), many Deliveries can be in flight at once; the driver renders each independently from its own timestamps — chaos at full speed reads as "the machine went off," and under slow-mo-hover a single inspected chain resolves cleanly (art doc).
...
- **Does not:** decide outcomes or timing (`Combat manager` / `combat_prd`); hold a clock (`Timekeeper`); advance combat; capture input (`UI`); render the corridor (the `docs/corridors/` renderer).
