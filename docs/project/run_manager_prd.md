...
- **The map** — the linear act/beat structure and the 1D progress track (boss at each act end, midpoint relic, plus rolled beats: basic fights, elites, events). Forward-visible, single, no branching ([design](design.md)). Counts/placements are design/tuning, not here.
...
3. **Roll the next beat** — a **ROLL beat** auto-selects COMBAT vs EVENT on the run RNG (anti-repeat biased) and draws a def from the matching pool; a **fixed beat** (boss, midpoint relic-grant) is next directly. The roll is the `Run manager`'s; the `Encounter` is the resolved unit ([Encounter PRD](encounter_prd.md)).
