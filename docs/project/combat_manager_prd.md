...
Orchestrator PRD. Sits under the [Architecture Map](architecture.md). The `Combat manager` is the **per-fight orchestrator** — it turns the [Combat PRD](combat_prd.md)'s settled resolution *model* into a running fight. Instanced one-per-fight by `Encounter`. It owns the live context, the [Timekeeper](timekeeper_prd.md) (the combat clock) lifecycle, the component registry, the central fixed-step tick, target-shape resolution, the trigger event bus, and win/loss.
...
- Not the resolution *model* — that's `combat_prd` (Ticker, fire → Delivery, travel, fizzle). The manager *executes* it.
...
2. **Fire** — each crossed item fires (combat_prd): reset its cooldown, emit a fire event, produce its payload(s). The manager resolves each payload's target-shape (below) and spawns a Delivery per target — `travel_time` 0 Deliveries land in step 3 this same step; others register and travel. Crossed *statuses* fire their payload (a status tick = a fire, travel 0); expired timed statuses are removed.
...
4. **Route events** — the event bus delivers this step's events (fires + lands) to subscribed trigger items, **pushing** their accumulators. **A push that fills a bar fires on the *next* step, not this one** — a chain advances at most one link per step (combat_prd's loop-proofness; no recursion within a step).
5. **Win/loss** — if a whole side is dead, signal the result and end (below).

**One fire pass per sim-step**, driven by crossings present at the advance; pushes land in accumulators but fire next step. That single-pass structure is what makes the cascade loop-proof *and* reproducible — the operational form of `combat_prd`'s accrual-only rule. (Headless autotest calls `sim_step()` directly in a loop — no `_physics_process`, no `steps_due`, no cap — so a run is "K sim-steps," bit-reproducible.)
...
- **opponent-leftmost** (single-target) → the leftmost living opponent relative to the owner's side. **Locked at spawn**; if it dies mid-flight the Delivery fizzles — no retarget (combat_prd).
...
**Actor-targeting** never gets smart — leftmost, no lowest-HP / highest-threat (combat_prd), for player predictability. **Item-targeting**'s single-target case is **random** (seeded) by deliberate choice — variety over predictability, to validate in testing. The *rules* are combat_prd's; their *runtime authority* (live ordering, the RNG draw) is here.
...
The event catalog and push amounts are content (the Item PRD: item declares; combat_prd: charges model). "Scales with item count" reads board state at resolve — a computed modifier, not an event.
...
- **throw-potion intent** → activates the consumable: builds its payload(s) (no Ticker — combat_prd), resolves the shape, spawns its Deliveries.
...
- **Executes** `combat_prd`'s resolution model.
