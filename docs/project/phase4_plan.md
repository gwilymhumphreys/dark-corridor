...
> ([ui_layout](ui_layout_prd.md) · [vfx_driver](vfx_driver_prd.md)) on top of the
> [architecture → Scene tree](architecture.md#scene-tree--node-model); this is the
> ordered, test-first path. Sits under [decision-log.md](decision-log.md) →
...
(`run.apply_draft_pick`) and resumes the loop. No skip (draft_prd). Localized.
...
`docs/index.md`, `handoff.md`, `decision-log.md` (build status), and add a
`docs/ui/run_screen.md` describing the framed composition + the real-time seam.
