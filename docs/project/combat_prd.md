...
"When X, do Y" is expressed as X pushing Y's accumulator (the charges model). There is no instant trigger-resolution and no deferred-resolve queue. Everything that "happens in response" happens by an accrual crossing its threshold on a tick. An instant reaction is just a push large enough to fill the bar (≈100%).
