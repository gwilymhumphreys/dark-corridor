...
|---|---|---|
| 1 | `Mechanic`, `MechanicRegistry`, attack, heal and shield. The `Delivery.Kind` change and the `mechanic` field. Convert all catalogs and tests. The colour renames. (Renaming block to shield, the colour renames, and the classes and registry are done.) | Registry lookups. An attack effect deals damage and publishes nothing new yet. A shield effect applies shield. An `APPLY_STATUS` with `'shield'` applies nothing. |
