...
| [decision_log.md](decision_log.md) | **The canonical decision record** (#1–#34): what was decided and why, the working approach, and what's open/deferred. Don't re-litigate anything in it. | decisions, rationale, settled, open questions, deferred, naming history |
...
| [systems/debug_panel.md](systems/debug_panel.md) | **The F1 debug panel** (dev-only autoload `DebugPanels`): palette clamp controls, the combat corridor renderer choice, enemy images (cut out, painted or pixel); start-up arguments for screenshots (`--palette`, `--corridor`, `--monster-image`); session-only, untranslated. | debug panel, F1, DebugPanels, autoload, dev tool, corridor renderer choice, enemy images, painted, cut out, start-up arguments, screenshots, reset_settings |
...
The first-person "dark corridor" renderer — three interchangeable renderers over a
shared base: two pseudo-3D from 2D pixel art, and one real 3D. **Read `common.md`
first**; it has the which-renderer guide, the file map, and the embedding panel.

| Doc | Covers | Keywords |
|-----|--------|----------|
| [systems/corridors/common.md](systems/corridors/common.md) | Shared base + host: the `CorridorRenderer` interface (incl. `axis_scale`, `enemy_brightness`), movement/blur/filter model, project settings, the testbed, the drop-in clipping panel, which renderer to use. | corridor, base class, CorridorRenderer, axis_scale, host, testbed, toggle, sharp bilinear, blur, velocity ramp, panel, clipping, view_size |
| [systems/corridors/corridor_3d.md](systems/corridors/corridor_3d.md) | **`Corridor3D`** (alternative). A SubViewport 3D scene with a fixed camera; lit by one OmniLight3D at the camera (range, energy, attenuation, flicker); enemy image brightness; sections placed from `player_z`, built by a swappable piece source (code-built quads or a bought kit). | 3D corridor, Corridor3D, SubViewport, Camera3D, OmniLight3D, centre light, light range, attenuation, light energy, flicker, enemy_brightness, section, piece source, CodeBuiltPieceSource, KitPieceSource, modular kit, depth zero |
