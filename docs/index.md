...
| [systems/palette_clamp.md](systems/palette_clamp.md) | **The full-screen palette clamp**: a screen shader snapping every pixel (HUD included) to a chosen palette; RGB or OKLab matching, ordered dithering; `PaletteLoader` for Lospec PNG strips and `.gpl` files. | palette, clamp, shader, hint_screen_texture, OKLab, perceptual, dithering, Bayer, gpl, Lospec, PaletteLoader, cohesion |
