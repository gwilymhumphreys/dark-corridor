...
| [systems/corridors/corridor_3d.md](systems/corridors/corridor_3d.md) | **`Corridor3D`** (alternative). A SubViewport 3D scene with a fixed camera; a shader light at the camera (range, smooth or banded falloff, flicker) drawn as a material overlay; sections placed from `player_z`, built by a swappable piece source (code-built quads or a bought kit). | 3D corridor, Corridor3D, SubViewport, Camera3D, corridor_light.gdshader, light range, banded falloff, flicker, material overlay, section, piece source, CodeBuiltPieceSource, KitPieceSource, modular kit, depth zero |
