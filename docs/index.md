...
| [systems/debug_panel.md](systems/debug_panel.md) | **The F1 debug panel** (dev-only autoload `DebugPanels`): palette clamp controls, F2 for the look panel; start-up arguments for screenshots (`--palette`, `--look`, `--corridor-set`, `--monster-image`); session-only, untranslated. | debug panel, F1, F2, DebugPanels, autoload, dev tool, corridor settings, start-up arguments, screenshots, reset_settings |
...
| [systems/background_wear.md](systems/background_wear.md) | **Background wear** (dev tool): a shader on each screen's background rectangle drawing print wear (specks, scratches, rubbed edges, creases) in two solid palette colours; `ScreenBackground`; settings in the F2 look panel and look files; `--background-set=`. | background wear, print wear, specks, scratches, edge wear, creases, record sleeve, ScreenBackground, background_material, background_wear.gdshader, UI_BACKGROUND_WEAR, --background-set |
