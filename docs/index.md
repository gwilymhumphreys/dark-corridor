...
| [systems/tooltips.md](systems/tooltips.md) | The combat item tooltip: hovering a board item opens a main panel (name, types, charge time, effect lines) and a keyword column beside it, with live values and inline keyword icons. Also the optional pop-up keyword tooltip a chip can carry elsewhere in the interface. |
| [systems/audio.md](systems/audio.md) | Audio: `SfxManager` (a sound is a folder of recordings named by path, one player per bus, per-key cooldown, pitch jitter), `MusicManager` (shuffle and crossfade), the bus layout, and the volume and silent-run defaults in `Prefs`. |
| [systems/cursor.md](systems/cursor.md) | The mouse cursor: the operating system's cursor is hidden and a pointing hand is drawn in its place, sized in canvas pixels and drawn through the interface look's picture material, with an outlined hand over clickable controls and a curled hand while the button is held. |
...
| [systems/testing.md](systems/testing.md) | How tests are written: GUT conventions, the per-area `tests/` folders, the fixture content that keeps tests off authored content and which tests may read the real content, `TestCleanup` for resetting autoloads between tests, and signal assertions. |
...
| [systems/item_browser.md](systems/item_browser.md) | The item browser: `tools/item_browser.sh` writes one web page of every item with its in-game tooltip text and icons, filterable by character, type, rarity and keyword. |
...
| [systems/dev_tools.md](systems/dev_tools.md) | Dev and test code kept out of the game in `src/debug/`: every start-up argument (screenshots, skipping the title, adding allies, board items and potions, look settings), the `Dev` autoload and `DevArgs`, the dev scenes, and the screenshot commands. |
| [systems/debug_panel.md](systems/debug_panel.md) | The dev-only `DebugPanels` autoload: one panel with a preset bar and seven tabs — F1 corridor, F2 interface and palette colours, F3 print, F4 background wear, F5 control feedback, F6 icon slots, F7 cardboard tokens. |
...
| [systems/ui_theme.md](systems/ui_theme.md) | The `dark_corridor` theme: flat palette-following panel and button styles with no pack art left, the chunky-UI-on-a-1440p-canvas approach, the `UI_SCALE` sizing unit, Rakkas as the default font, and the seven-rung text ladder behind the text size slider. |
...
| [design/smith.md](design/smith.md) | Smith working file (the starter character, absorbing the former Armourer): the low-load anchor, weapons/armour/skills with no second resource, go wide and go tall, the armour stack-and-spend engine and the built empower engine. |
