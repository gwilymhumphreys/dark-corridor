...
| [project/spore_engine_prd.md](project/spore_engine_prd.md) | **Spore Engine Support PRD** (engineering, NOT content — deferred capability for a building agent). The engine seams the first status-identity character (the Mushroom Druid, `docs/design/`) needs beyond apply/tick/resolve: (1) **status-stack consumption** (`StatusManager.consume` — spend spores as fuel; self-fuel in the Item pipeline, opponent-fuel in the Combat manager spawn path), (2) **evasion** (the "acts but misses" fizzle seam for blinding, with a fizzle-reason — distinct from silence/gate), (3) the **player-side consumer** of the already-deferred mid-fight roster add (summon tokens / lethal's spawn rider). Marks the appliers as needing nothing new; numbers stay content. | spore engine, status consumption, consume stacks, Mass, fuel, evasion, blinding, miss, whiff, fizzle reason, summon, mid-fight roster, token, body-block, lethal execute, Mushroom Druid, engine work, deferred capability |
