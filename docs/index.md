...
| [systems/debug_panel.md](systems/debug_panel.md) | **The F1 debug panel** (dev-only autoload `DebugPanels`): palette clamp controls, F2 for the look panel, F3 for the print panel; start-up arguments for screenshots (`--palette`, `--look`, `--corridor-set`, `--monster-image`); session-only, untranslated. | debug panel, F1, F2, F3, DebugPanels, autoload, dev tool, corridor settings, start-up arguments, screenshots, reset_settings |
...
| [systems/background_wear.md](systems/background_wear.md) | **Background wear** (dev tool): a shader on each screen's background rectangle drawing print wear (faded areas, specks, rubbed edges, creases, folds) in palette colours, shared with the corridor overlay through `print_wear.gdshaderinc`; `ScreenBackground`; settings in the F3 print panel and look files; `--background-set=`. | background wear, print wear, faded areas, specks, edge wear, creases, folds, print_wear.gdshaderinc, record sleeve, ScreenBackground, background_material, background_wear.gdshader, UI_BACKGROUND_WEAR, --background-set |
| [systems/print_frame.md](systems/print_frame.md) | **Print frame** (dev tool): the F3 print panel; `PrintFrame` moves the combat corridor in by a margin and draws a rough printed border behind it and an overlay on it that carries the background wear across the corridor and wears its edge away; two-ink red look files. | print frame, print panel, F3, PrintFrame, border, corridor margin, corridor overlay, worn edge, folded paper, print_border.gdshader, corridor_overlay.gdshader, UI_BORDER, --print-set, two-ink, halftone, hatching |
| [systems/interface_palette.md](systems/interface_palette.md) | **The interface palette** (dev tool): a named `.gpl` file replaces `Colours` variables and recolours the theme's font colours, images and panel colours by brightness; `NamedColourRect` for colours placed in scenes; `--ui-palette=` and the F1 panel. | interface palette, ui palette, Colours, colours.gd, static var, gpl, named colours, theme recolour, NamedColourRect, ui-default, ui-muted, --ui-palette |
| [systems/ui_theme.md](systems/ui_theme.md) | **UI theme, pixel-scale & fonts.** The Black & White theme + its panel variations; the ~360p-chunky-UI-on-a-1440p-canvas approach (canvas_items, Nearest, why not a low-res viewport); the `UI_SCALE` sizing unit + integer-at-rest/smooth-in-motion rule; the vector↔pixel font toggle via `Prefs`. | theme, ui scale, pixel art, UI_SCALE, canvas_items, StyleBoxTexture, expand_margin, default_base_scale, font, vector, pixel, oversampling, FontStyle, Prefs, integer at-rest, panel variation |
...
| [design/armourer.md](design/armourer.md) | **Armourer — working file** (the on-ramp / starter character). The low-load anchor (successor to the Wanderer placeholder); block stack/spend engine; ~3 overlapping archetypes; block-as-fuel rides the built consume seam. | armourer, character, starter, on-ramp, anchor, block, stack spend, armour, affordance, Ironclad, low-load, Wanderer successor |
