...
| [systems/debug_panel.md](systems/debug_panel.md) | **The F1 debug panel** (dev-only autoload `DebugPanels`): palette clamp controls, the combat corridor renderer choice, painted vs pixel enemy images; session-only, untranslated. | debug panel, F1, DebugPanels, autoload, dev tool, corridor renderer choice, enemy images, painted, reset_settings |
...
| Doc | Covers | Keywords |
|-----|--------|----------|
| [systems/corridors/common.md](systems/corridors/common.md) | Shared base + host: the `CorridorRenderer` interface (incl. `axis_scale`), movement/blur/filter model, project settings, the testbed, the drop-in clipping panel, which renderer to use. | corridor, base class, CorridorRenderer, axis_scale, host, testbed, toggle, sharp bilinear, blur, velocity ramp, panel, clipping, view_size |
