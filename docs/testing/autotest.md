...
- **The driver is just another input-intent source.** The [input/output split](../project/architecture.md) has the `UI` emit *intents* (draft-pick, potion-throw, choice-select, timescale); the autotest driver emits the **same** intents headlessly. Combat is already automatic, so the driver only makes the few human decisions — it never "plays" a fight.
...
- **Determinism is seeded RNG.** [Save](../project/save_prd.md) already snapshots RNG state; the harness seeds it (`--seed`). The tick's determinism constraint makes runs reproducible.
...
- **`AutoTestMode`** — entry: parse flags, seed RNG, set the dial, force `--nosave --notutorial`, own the run lifecycle + game-time / wall-time timeouts + exit code.
...
- **Timekeeper dial** → `--speed`. **Seedable RNG** (Save snapshots it) → `--seed`.
- **Run manager** drives the encounter sequence the harness steps through and exposes run state for logging.
