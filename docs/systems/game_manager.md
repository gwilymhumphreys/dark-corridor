...
Session PRD (top of the tree). Sits under the [Architecture Map](architecture.md). The `Game manager` is the **session singleton** — it owns the game-state machine, the run lifecycle, and the save-lifecycle calls, and is the reachable-from-anywhere coordinator. It sits *above* the [Run manager](run_manager.md): **Game (session) → Run (descent) → Combat (fight)**.
