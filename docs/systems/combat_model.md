...
"When X, do Y" is expressed as X pushing Y's accumulator (the charges model). There is no instant trigger-resolution and no deferred-resolve queue. Everything that "happens in response" happens by an accrual crossing its threshold on a tick. A trigger is authored as the seconds it charges its item (`trigger_subs` → `seconds`); an instant reaction is just a push as long as the whole cooldown. "When X" defaults to "when **my side** does X" (decision #30) — events carry source identity, and a trigger opts into `ANY` / opponent-side listening per item.
...
Every Delivery an item fires travels the same fixed number of steps (`Balance.TRAVEL_STEPS`), whatever it carries and wherever it goes: shields and heals on the holder, charges on the holder's own items, summons and created items all fly a projectile (decision #48). Nothing an item fires lands on the step it was fired, and the visuals draw the flight from the step count. Thrown potions go through the same loop but fly only `Balance.POTION_TRAVEL_STEPS` (one step), so they land on the next step: a potion is used in a crisis, often in slow motion, and a delayed heal would feel bad. Because a charge an item aims at itself lands a full travel later, an item can target itself without firing every step.
