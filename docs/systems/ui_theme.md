...
| `PanelToken` | item cells (`item_cell.tscn`, which the potion slots also use), and the combat portraits when the `token_portraits` print setting is on: `PanelSlot` with a drop shadow, so items read as cardboard tokens on the board grid. The shadow and fill are set by `PrintLook.apply_token_style()` ([print_frame.md](print_frame.md)) |
| `PanelTokenWide` | the same with wider margins: the character panels (player, allies and enemies, `character_panel.tscn`) when the `portrait_panel` print setting is on |
