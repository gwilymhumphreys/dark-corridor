...
- **Reads (to draw):** actor / board / item / status / potion state + the `Run manager`'s map / run-state; composes over the `docs/systems/corridors/` corridor renderer and the [VFX driver](vfx_driver.md)'s combat wall.
