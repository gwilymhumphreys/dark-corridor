...
- **Active (push)** — the status acts via `ctx`: `on_apply`, `on_expire`, `on_step(target, ctx) -> expired`, `on_holder_fired(item, ctx)` (an **item-targeted** status acts when its holder item fires — Decay drains here, [`item_creation_and_decay.md`](item_creation_and_decay.md)), `on_owner_item_fired(actor, item, ctx) -> expired` (the **actor-targeted** twin — acts when one of that actor's items fires; Bleed cashes out here, [`mechanic_ideas.md`](../design/mechanic_ideas.md)), `setup` / `reapply` (lifecycle + stacking). The **firing `item`** is passed so a status can scope to a weapon attack (the Armourer empower spends a charge here — #35); Bleed ignores it. This is the **real-fire** path (not the tooltip preview), so state-consuming work belongs here. `on_expire` runs at every **natural** removal — timed expiry, consumed-to-zero, spent-removal after a damage pass — but **not** at combat teardown (the fight ending is a clear, not an expiry).
- **Modifiers (pull)** — the engine queries at a pipeline stage, in list order: `modify_outgoing(amount, target, item, ctx)`, `modify_incoming`, `absorb(amount, flags, …) -> remaining`, `gates_fire`, `causes_evasion`. Plus `is_fuel` / `consume` (Mass), `is_spent` (pool removal), and presentation fields. `modify_outgoing` also receives the **firing `item`** (optional, default `null`) so a status can scope to a weapon attack (Weak scales any; the empower doubles only `weapon`-tagged damage — #35); it **must stay pure** because it also runs on the read-only tooltip-preview path (`Item.display_value`) — the charge-spend lives on `on_owner_item_fired`, not here.
...
- Ten statuses across the shapes: **shield** (pool), **poison** (periodic DoT), **weak** / **vulnerable** / **blind** (timed), **silence** (static gate), **spores** (inert counter / Mass fuel), **decay** (item-targeted use-status — drained by the holder's fire, removes the item at 0; [`item_creation_and_decay.md`](item_creation_and_decay.md)), **bleed** (actor-targeted — drained by the holder's *own* item fires for a self-paying-down wound; an enemy-applied debuff, [`mechanic_ideas.md`](../design/mechanic_ideas.md)), **empowered** (actor-targeted consumed counter — doubles a `weapon` attack; `modify_outgoing` reads it (pure), `on_owner_item_fired` spends one charge per weapon attack; the Armourer's Mighty Blow, #35, [`armourer.md`](../design/armourer.md)).
