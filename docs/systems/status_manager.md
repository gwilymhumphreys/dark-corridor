...
- `count / stacks` — a whole number (`int`). `setup`, `reapply`, `consume` and `StatusManager.reduce` round the amount they are given, so callers can pass a computed value (decision #49).
...
4. **On-apply event** is emitted at the Delivery's land (`APPLIED` carries the mechanic id, or the status id for an `APPLY_STATUS` delivery — [mechanics.md](mechanics.md)) — **only when the apply succeeded** (an unknown id applies nothing and publishes nothing) — so reactive items can trigger ("when you apply poison, gain 1 shield").
...
- **Shield multipliers** ([mechanics.md](mechanics.md)): the `mechanic_id` argument names the mechanic that dealt the damage, and the shield pool spends the damage times that mechanic's multiplier — `MechanicRegistry.shield_multiplier(mechanic_id)` (1.0 for an empty or unknown id). Poison returns double; burn and bleed return half (their `SHIELD_MULTIPLIER_*` from `balance.gd`). The calculation keeps the pool whole: the cost is `roundi(net * m)`; a pool that can pay it loses the cost and passes nothing on, and one that cannot covers `count / m` damage, drops to 0 and passes `net - count / m` on (`take_damage` rounds it). Every caller passes its mechanic: the attack mechanic passes `'attack'`, `PeriodicStatus` passes its own `id`, and bleed passes `'bleed'`.
- **Stat-statuses — BUILT (#6).** `VulnerableStatus` overrides `modify_incoming` (amplifier — scales up before shield) and `WeakStatus` overrides `outgoing_bonus` (applied to the holder's attack payloads **at fire time** in `Item._resolve_effect`, through `StatusManager.outgoing_bonuses` and `StatusManager.combine` — [mechanics.md → Combining bonuses](mechanics.md#combining-bonuses)). Magnitudes are **% multipliers**, not flat-per-fire (a flat per-fire modifier makes fast items strictly dominant — the authoring guidance). The real stat-status content (numbers, per-stack variants) is the owner's. The `item` arg (#35) lets a modifier scope to a type of item (none does at the moment).
...
- **Active (push)** — the status acts via `ctx`: `on_apply`, `on_expire`, `on_step(target, ctx) -> expired`, `on_holder_fired(item, ctx)` (an **item-targeted** status acts when its holder item fires — Decay drains here, [`item_creation_and_decay.md`](item_creation_and_decay.md)), `on_owner_item_fired(actor, item, ctx) -> expired` (the **actor-targeted** twin — acts when one of that actor's items fires; the Smith empower uses up a stack here, #35), `on_holder_attacked(target, ctx) -> expired` (acts when an **attack lands on the holder** — Bleed bites here, [`mechanics.md`](mechanics.md); poison/burn ticks, the status's own damage and outside-set damage never call it), `setup` / `reapply` (lifecycle + stacking). The **firing `item`** is passed to `on_owner_item_fired` so a status can scope to a weapon attack (the Smith empower uses up a stack here — #35). This is the **real-fire** path (not the tooltip preview), so state-consuming work belongs here. `on_expire` runs at every **natural** removal — timed expiry, consumed-to-zero, reduced-to-zero (`reduce`), spent-removal after a damage pass — but **not** at combat teardown (the fight ending is a clear, not an expiry).
- **Modifiers (pull)** — the engine queries at a pipeline stage, in list order: `outgoing_bonus(target, item) -> {'flat', 'percent'}`, `modify_incoming`, `absorb(amount, flags, target, ctx, mechanic_id) -> remaining`, `gates_fire`, `causes_evasion`. Plus `is_fuel` / `consume` (Mass), `is_spent` (pool removal), and presentation fields. `outgoing_bonus` reports a bonus instead of changing the value, so `StatusManager.combine` can apply every bonus by one rule. It is asked of the owner's statuses and of the firing item's own statuses, and also receives the **firing `item`** (optional, default `null`) so a status can scope to a weapon attack (Weak and the empower scale any attack — #35); it **must stay pure** because it also runs on the read-only tooltip-preview path (`Item.display_value`) — using up a stack lives on `on_owner_item_fired`, not here.

**Thrown consumables are exempt from the holder's combat modifiers** (decision #30): a potion's payload skips the status bonuses (Weak) and `has_evasion` (Blind) — potions are the reserve, not the engine, so debuffs that degrade the board don't degrade the panic button. A deliberate asymmetry with the item fire pipeline, not an accident.
