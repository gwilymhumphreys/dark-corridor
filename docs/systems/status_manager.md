...
- **Shield multipliers** ([mechanics.md](mechanics.md)): the `mechanic_id` argument names the mechanic that dealt the damage, and the shield pool spends the damage times that mechanic's multiplier — `MechanicRegistry.shield_multiplier(mechanic_id)` (1.0 for an empty or unknown id). Poison, burn and bleed will return `SHIELD_MULTIPLIER_*` from `balance.gd` once their mechanic classes exist; until then every hit uses 1.0. The calculation is `covered = min(net, count / m); count -= covered * m; net -= covered`. Every caller passes its mechanic: the attack mechanic passes `'attack'`, `PeriodicStatus` passes its own `id`, and bleed passes `'bleed'`.
