...
- **own-item-random** → one random item on the owner's own board, drawn from the same seeded RNG (`_pick_random`). The firing item is excluded from the pool.
