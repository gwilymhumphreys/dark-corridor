...
- **At resolution:** every registered item's cooldown is reset to zero again, so the board is not left part-charged between fights (the view also hides the cooldown fills once the fight is over — [run_screen.md](run_screen.md#overlays)).
...
1. **Advance** — `timekeeper.advance()` (clock += `STEP`); advance every registered component one step (item cooldowns filled by time and/or earlier pushes; periodic/timed statuses; Delivery travels). Collect the items and Deliveries that **cross**. A crossed *status* applies its tick effect directly inside `on_step` (a DoT damages its holder there); the manager fabricates a pre-landed **visual-only** Delivery so the wall still shows the number. Expired timed statuses are removed (their `on_expire` hook runs).
2. **Land** — the Deliveries that arrived this step apply their result: a `MECHANIC` delivery calls its [mechanic's](mechanics.md) `land` (attack: `Actor.take_damage` through the `StatusManager` modifier pipeline; heal; shield: `StatusManager.apply`), an `APPLY_STATUS` delivery calls `StatusManager.apply`. An `APPLY_STATUS` naming a mechanic's status is an authoring error and applies nothing. Each landing that applies publishes one `APPLIED` event carrying the mechanic or status id ([mechanics.md](mechanics.md#the-applied-event)).
3. **Reap** — dead enemies and summoned tokens leave combat, so nothing fired this step targets them.
4. **Win/loss** — if a whole side is dead, signal the result and end (below). Nothing fires on the step a fight is decided.
5. **Fire** — each crossed item fires (combat_model.md): reset its cooldown, emit a fire event, produce its payload(s). An item removed earlier in the step, or whose owner died, does not fire. The manager resolves each payload's target-shape (below) against the board as this step's landings left it, and spawns a Delivery per target, which lands `Balance.TRAVEL_STEPS` later.

Landing comes before firing so a target that dies this step is not picked (decision #48). Events from lands and fires are routed inline: the bus **pushes** the accumulators of subscribed trigger items, but crossings were already collected in step 1, so **a push that fills a bar fires on the *next* step, not this one** — a chain advances at most one link per step (combat_model.md's loop-proofness; no recursion within a step).
...
- **own-item-random** → one random item on the owner's own board, drawn from the same seeded RNG (`_pick_random`). The firing item is in the pool.
- **all-own-items** → every item on the owner's own board (`_all_own_items`), the firing item included. The owner's board only, not an ally's.

**Pool, then filter, then pick**. The four item shapes are built from three steps: a pool builder (`_all_opponent_items` / `_all_own_items`), an optional **target filter** (`_filter_items`) that drops the pool items a payload's `target_filter` rejects, and the pick (`_pick_random` for the two `-random` shapes). The filter is applied **before** the draw, so a filtered random pick is decided by the filter, not the RNG; an unfiltered shape (a null or empty filter) resolves exactly as it did before the filter step was added. An empty result after filtering yields no Delivery, like an empty pool today. The draw stays **flat across items, not across enemies** (an enemy holding four items is four times as likely to be picked as one holding one) — `_pick_random`'s comment says so. The three actor shapes (`self`, `opponent-leftmost`, `all-opponents`) apply **no** filter: a filter on one is an authoring mistake, warned once per shape (`_warn_ignored_filter`) and then ignored.
...
The manager holds an **optional** `var combat_log: CombatLog = null` ([combat_log.md](combat_log.md)). The run screen / the autotest assign one after `start()`; the sandbox + most tests leave it null. At each mutation site the manager makes one **null-guarded** direct call (not via the [event bus](#the-trigger-event-bus) — the listener channel carries no amount and no timestamp, and DoT publishes no event). Six sites + throws: **fire** (`_fire_item`, after `ITEM_FIRED`), **direct damage** + **heal** (`AttackMechanic.land` / `HealMechanic.land`, capturing `Actor.take_damage` / `heal`'s returned HP delta), **shield** (the base `Mechanic.land`) and **other statuses** (`_land`'s `APPLY_STATUS` branch), **DoT damage** (`_advance_statuses_on`, where the tick's `dealt` is already computed for the visual — `st.source` resolves Item → name+owner-side, Actor → side, null → the holder's opponent) and **status healing** (the same pass reacts to a status's health **gain** — a regen tick — logging a heal with the status's `name_key` as the source), **attack-triggered status damage** (`_on_holder_attacked`, called from the attack mechanic's `land` when the target survives — bleed's bite; the shared `_show_status_damage` helper writes the wall visual + log for the DoT, fire-status and attack-triggered paths alike), and **throws** (`throw_consumable`). Source identity + side are resolved at the write site (`_delivery_source_*` / `_status_source_*` / `_side_of`) into `name_key`s + side ints — no game-object refs are stored. `teardown()` is unaffected (the log is the caller's to retain for a post-fight screen).
...
- **Mid-fight item changes (creation + decay) — BUILT.** `add_item(actor, def_id)` is the item-level cousin of `add_actor`: it puts a new `Item` on a live board (registers its Ticker/triggers, seeds its decay), resolved at a `Delivery.Kind.CREATE_ITEM` land (shape SELF → the firer's own board). Created items are **combat-scoped** (tracked in `_created_items`, mirroring `_player_tokens`): teardown strips them from the (possibly run-scoped) board and dissolves them, so the run snapshot never serializes one. `remove_item(item)` is the individual-live-item removal — drop from board + sweep, unsubscribe, dissolve — which the **Decay** use-status calls (via a minimal `StatusContext`) when it empties. See [`item_creation_and_decay.md`](item_creation_and_decay.md).
