...
2. **Fire** — each crossed item fires (combat_model.md): reset its cooldown, emit a fire event, produce its payload(s). The manager resolves each payload's target-shape (below) and spawns a Delivery per target — `travel_time` 0 Deliveries land in step 3 this same step; others register and travel. Crossed *statuses* fire their payload (a status tick = a fire, travel 0); expired timed statuses are removed.
