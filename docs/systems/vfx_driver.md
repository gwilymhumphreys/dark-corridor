...
motion and pause keep working. The driver keeps a dictionary from `Delivery.Kind` to the drawer for
that kind, so a new effect is a new file rather than another branch in `_draw()`. Damage, healing
