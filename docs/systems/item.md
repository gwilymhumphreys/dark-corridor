...
- **own-item-random** — one *random* item on the owner's own board, for the [charge and decharge mechanics](mechanics.md#charge-and-decharge). Same seeded RNG as the opponent case. The firing item is in the pool, so an item can charge itself; the charge lands a full travel after the fire ([combat_model.md](combat_model.md#fire--delivery)).
- **all-own-items** — every item on the owner's own board, the firing item included. Only the owner's board — an ally's items are not included.
