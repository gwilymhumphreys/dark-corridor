...
| `--strategy S` | draft strategy: `first-viable` (default) · `random` · `damage` / `shield` / `poison` (family) · `greedy-synergy` (+ `scaling`/`burn` aliases). **Live** (seeded). |
