...
- **`AutoTestLogger`** (`auto_test_logger.gd`) — structured events (run / encounter / draft / fight start+end) + a damage-by-family tally + **per-encounter records** (duration vs the ~10–15s window, HP before→after, outcome) + **per-item contribution** (player board: fires + damage, with a never-fired item flagged a **trap pick** — fire-count, not damage, is the "did it do anything" signal so block/heal items aren't false-flagged; fire counts are **exact**, observed off the event bus's `ITEM_FIRED` listener channel rather than inferred from cooldown movement; **block applied + healing landed** are tallied per item, so defensive items are rankable) + a summary + a markdown report stamped with its **seed + strategy** (incl. an *Encounters* table and an *Item contribution* table — what `tune` reads after every run). Observation covers the **live rosters** — allies + summon tokens included, for damage, DoT attribution, and the stuck guard. Reads handed state, writes no game state. Damage-by-family is built per step from net HP loss + the Deliveries that landed that step: direct hits → the source item's family; the unexplained remainder is **DoT credited to the item that applied it** (poison ticks land no Delivery) via a **pre-step status snapshot** of each actor's DoT-applying statuses (their `source` item + potential tick damage). So a poison item (Venom Fang) shows its damage in the contribution table instead of reading 0, and a multi-applier remainder splits between them by weight; a source-less DoT falls back to the generic `Poison` channel.
