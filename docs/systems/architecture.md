...
### `Timekeeper` — PRD: [timekeeper.md](timekeeper.md)
...
### `Combat manager` — PRD: [combat_manager.md](combat_manager.md)
...
### `StatusManager` — PRD: [status_manager.md](status_manager.md)
 
- **Exposes:** `apply(target, id, count, duration?, source?, flags?, ctx?) → StatusEffect` (find-and-reapply or build via `StatusRegistry`; the class decides stacking; duration rides the application); `resolve_incoming_damage(target, raw, flags) → net` (folds each status's `modify_incoming` — amplifiers like Vulnerable — then `absorb` — shield); `outgoing_bonuses(actor, item)` + `combine(value, bonuses)` (the outgoing bonuses from the owner's and the item's statuses, like Weak, combined at fire); `consume(target, id, amount) → stacks-removed` (spend a fuel status — spore_engine Cap 1); `has_evasion(actor)` (any status `causes_evasion()` — spore_engine Cap 2); `advance_status`. The item-targeted **use-status** (Decay) drains via the `StatusEffect.on_holder_fired(item, ctx)` active hook — its emptying removes the host item through a minimal `StatusContext.remove_item` the Combat manager fulfils (item_creation_and_decay Cap 2). **Stateless** — holds no instances; behaviour is in the `StatusEffect` classes.
...
### `Encounter` — PRD: [encounter.md](encounter.md)
...
### `UI` — PRD: [ui_layout.md](ui_layout.md)
...
### `VFX driver` — PRD: [vfx_driver.md](vfx_driver.md)
...
- **Does not:** decide outcomes or timing (`Combat manager`); hold a clock (`Timekeeper`); cause damage (the Delivery's landing does); render the corridor (`docs/systems/corridors/`).
